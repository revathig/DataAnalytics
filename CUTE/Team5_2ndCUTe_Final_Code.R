...
## The highest adjusted GVIF is of years of education @ 5.97
...
vif(log_reg2) ## Highest adjusted GVIF for relationship @ 5.35
...
print(sensitivity) ## Sensitivity at 0.59

## Accuracy

accuracy <- sum(diag(conf_matrix))/sum(conf_matrix)

print(accuracy) ## Accuracy at 0.8523 on validation
...
## Grouping Values for Working Sector ##
...
## Grouping Values for Qualification
...
## Grouping Values for Marital Status ##
...
## Grouping Values for Occupation ##
...
traintemp<-train_data[,-14]

testdatatemp<-knnImputation(data=testdatatemp, k=5, distData = traintemp)

rm(traintemp) ## Remove train temp variable
...
write.csv(preds_test,file="Group5_TestPred.csv") ## Accuracy is 84.53%
myoutput = read.csv("Group5_TestPred.csv", header =T)
str(myoutput)
